...
/obj/structure/table/use_weapon(obj/item/weapon, mob/user, list/click_params)
	...
	if (isWelder(weapon))
		...
		if (istype(weapon, /obj/item/weldingtool) && !welder.can_use(1, user, "to repair \the [src]"))
			return TRUE
		...
		if (!user.do_skilled((weapon.toolspeed * 2) SECONDS, SKILL_CONSTRUCTION, src, do_flags = DO_REPAIR_CONSTRUCT) || !user.use_sanity_check(src, weapon) || (istype(weapon, /obj/item/weldingtool) && !welder.remove_fuel(1)))
			return TRUE
		playsound(src, 'sound/items/Welder.ogg', 50, TRUE)
