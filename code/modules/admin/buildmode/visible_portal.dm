...
/obj/map_effect/temp/portal_marker
	invisibility = INVISIBILITY_OBSERVER
// [FD-EDIT] - Не полноценно перенес, а лишь рваными полотнами. В-прочем, это все равно должно работать, по этому забью до мерджа с Бэем
...
/datum/build_mode/visible_portals/proc/AddMarker(turf/T, dir, entry)
	var/obj/map_effect/temp/portal_marker/existingMarker = locate() in T
	var/obj/map_effect/portal/existingPortal = locate() in T
	if (existingPortal || existingMarker)
		to_chat(user, SPAN_WARNING("Theres something already there!"))
		return

	var/obj/map_effect/temp/portal_marker/PM = new(T)
	PM.dir = dir
	if (entry)
		PM.icon_state = "portal_line_side_a"
		if (LAZYLEN(entrance))
			var/obj/map_effect/temp/portal_marker/M = LAZYACCESS(entrance, 1)
			PM.dir = M.dir
		LAZYADD(entrance, PM)
	else
		PM.icon_state = "portal_line_side_b"
		if (LAZYLEN(exit))
			var/obj/map_effect/temp/portal_marker/M = LAZYACCESS(exit, 1)
			PM.dir = M.dir
		LAZYADD(exit, PM)

/datum/build_mode/visible_portals/proc/FinishPortals()
	for (var/i = LAZYLEN(entrance) to 1 step -1)
		var/obj/map_effect/temp/portal_marker/A = LAZYACCESS(entrance, i)
		var/obj/map_effect/temp/portal_marker/B = LAZYACCESS(exit, i)
		var/turf/T1 = get_turf(A)
		var/turf/T2 = get_turf(B)
		if (i == 1)
			var/id = "\ref[A]"
			var/obj/map_effect/portal/master/MA  = new (T1, id, A.dir)
			var/obj/map_effect/portal/master/MB  = new (T2, id, B.dir)
			MA.assemble()
			MB.assemble()
		else
			var/obj/map_effect/portal/line/LA = new (T1)
			LA.dir = A.dir
			var/obj/map_effect/portal/line/LB = new (T2)
			LB.dir = B.dir
...
/datum/build_mode/visible_portals/OnClick(atom/A, list/parameters)
	...
	if (parameters["ctrl"])
		...
		if (parameters["right"])
			...
			var/obj/map_effect/portal/existingPortal = locate() in location
			if (existingPortal)
				var/obj/map_effect/portal/master/master = null
				if (istype(existingPortal, /obj/map_effect/portal/master))
					master = existingPortal
				else
					var/obj/map_effect/portal/line/line = existingPortal
