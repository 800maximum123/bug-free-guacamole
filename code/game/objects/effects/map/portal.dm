...
Portals do have some specific requirements when mapping them in;
	- There must by one, and only one `/obj/map_effect/portal/master` for each side of a portal.
	...
	- Portals can be made to be longer than 1x1 with `/obj/map_effect/portal/line`s,
...
/obj/map_effect/portal
	...
	var/obj/map_effect/portal/counterpart = null
...
/obj/map_effect/portal/Crossed(atom/movable/atom)
...
/obj/map_effect/portal/proc/go_through_portal(atom/movable/atom)
 ...
 */
/obj/map_effect/portal/proc/get_focused_turf()
...
/obj/map_effect/portal/proc/calculate_dimensions()
	var/highest_x = 0
	var/lowest_x = 0
...
/obj/map_effect/portal/master/Initialize(mapload, _id, _dir)
	if (!isnull(_id))
		portal_id = _id
...
/obj/map_effect/portal/master/proc/assemble()
	find_counterparts()
	make_visuals()
	apply_offset()

/obj/map_effect/portal/master/LateInitialize()
	if(!manual_initialization)
		assemble()

/obj/map_effect/portal/master/Destroy()
	GLOB.all_portal_masters -= src
...
/obj/map_effect/portal/master/proc/find_lines()
	...
	for(var/dir_to_search in dirs_to_search)
		...
		while(current_T)
			...
			var/obj/map_effect/portal/line/line = locate() in current_T
			if(line)
				portal_lines += line
				line.my_master = src
			else
...
/obj/map_effect/portal/master/proc/find_counterparts()
	for(var/thing in GLOB.all_portal_masters)
		var/obj/map_effect/portal/master/M = thing
		if(M == src)
			continue
		if(M.counterpart)
			continue
		...
		if(M.portal_id == src.portal_id)
			...
			if(length(portal_lines))
				for(var/i = 1 to length(portal_lines))
					var/obj/map_effect/portal/line/our_line = portal_lines[i]
					var/obj/map_effect/portal/line/their_line = M.portal_lines[i]
...
/obj/map_effect/portal/master/proc/make_visuals()
	var/list/observed_turfs = list()
	for(var/thing in portal_lines + src)
		var/obj/map_effect/portal/P = thing
		P.name = null
		P.icon_state = null
...
/obj/map_effect/portal/master/proc/apply_offset()
	for(var/thing in portal_lines + src)
		var/obj/map_effect/portal/P = thing
...
/obj/map_effect/portal/master/see_emote(mob/M, text)
	if(!counterpart)
		return
	var/turf/T = counterpart.get_focused_turf()
...
/obj/map_effect/portal/master/show_message(msg, type, alt, alt_type)
	if(!counterpart)
		return
	var/turf/T = counterpart.get_focused_turf()
	var/list/mobs_to_relay = list()
...
/obj/map_effect/portal/master/hear_talk(mob/speaker, text, verb, datum/language_speaking)
	if(!counterpart)
		return
...
// /obj/map_effect/portal/master/proc/get_apparent_position(atom/A)
// 	if(!counterpart)
// 		return null

// 	var/turf/true_turf = get_turf(A)
// 	var/obj/map_effect/portal/master/other_master = counterpart
...
/obj/map_effect/portal/master/side_a
	name = "portal master A"
	icon_state = "portal_side_a"

/obj/map_effect/portal/master/side_b
...
/obj/map_effect/portal/line
	name = "portal line"
	var/obj/map_effect/portal/master/my_master = null

/obj/map_effect/portal/line/Destroy()
	if(my_master)
		my_master.portal_lines -= src
		my_master = null
	return ..()

/obj/map_effect/portal/line/side_a
	name = "portal line A"
	icon_state = "portal_line_side_a"

/obj/map_effect/portal/line/side_b
	name = "portal line B"
