...
/datum/storage_ui/default/proc/arrange_item_slots(rows, cols)
	var/cx = 2
	var/cy = 3+rows
	boxes.screen_loc = "2:16,3:16 to [2+cols]:16,[3+rows]:16"
	...
	closer.screen_loc = "[2+cols+1]:16,3:16"
...
/datum/storage_ui/default/proc/space_orient_objs()
	...
	storage_start.screen_loc = "2:16,3:16"
	storage_continue.screen_loc = "2:[storage_cap_width+(storage_width-storage_cap_width*2)/2+2],3:16"
	storage_end.screen_loc = "2:[19+storage_width-storage_cap_width],3:16"
	...
	for(var/obj/item/O in storage.contents)
		...
		O.screen_loc = "2:[round((startpoint+endpoint)/2)+2],3:16"
		O.maptext = ""
		O.hud_layerise()

	closer.screen_loc = "2:[storage_width+19],3:16"
