...
/proc/everyone_look_at_manta(anim_time = 10 SECONDS, reset_delay = 20 SECONDS, anim_distance = 150)
	set waitfor = FALSE

	anim_distance *= 32

	for(var/obj/overmap/visitable/ship/bomber as() in GLOB.known_overmap_sectors)
		bomber.invisibility = 0
	var/list/affected_mobs = list()
	for(var/client/client as() in GLOB.clients)
		ADD_TRANSFORMATION_MOVEMENT_HANDLER(client.mob)
		sound_to(client.mob, sound('sound/ambience/ominous3.ogg', volume = 60))
	...
	for(var/obj/overmap/visitable/ship/bomber as() in GLOB.known_overmap_sectors)
		bomber.invisibility = INVISIBILITY_OVERMAP
	for(var/client/client as() in GLOB.clients)
		client.pixel_y = anim_distance
		client.adminobs = null
		client.mob.reset_view(null)
