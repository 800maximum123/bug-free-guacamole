// IPC BODY MARKINGS

/datum/sprite_accessory/marking/ipc/head/b_selen
	name = "Bishop Selen Head Primary Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "selen_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/b_sienna
	name = "Bishop Sienna Head Primary Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "sienna_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/b_siseyo
	name = "Bishop Siseyo Head Primary Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "siseyo_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/b_mask
	name = "Bishop Face Mask"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_mask"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/b_mask/triangular
	name = "Bishop Triangular Face Mask"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_triangular_mask"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/b_knight
	name = "Bishop Knight Head Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_panels"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/torso/b_knight
	name = "Bishop Knight Torso Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_panels"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/arms/b_knight
	name = "Bishop Knight Arm Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_panels"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/legs/b_knight
	name = "Bishop Knight Leg Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_panels"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/b_knight_lights
	name = "Bishop Knight Head Lights"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_lights"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/torso/b_knight_lights
	name = "Bishop Knight Torso Lights"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_lights"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/arms/b_knight_lights
	name = "Bishop Knight Arm Lights"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_lights"
	blend = ICON_MULTIPLY
	body_parts = list(BP_L_ARM,BP_L_HAND)

/datum/sprite_accessory/marking/ipc/legs/b_knight_lights
	name = "Bishop Knight Leg Lights"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_bishop.dmi'
	icon_state = "bishop_lights"
	blend = ICON_MULTIPLY
	body_parts = list(BP_R_FOOT,BP_L_FOOT)

/datum/sprite_accessory/marking/ipc/head/zenghu_panel
	name = "Zeng-Hu Spirit Head Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_zenghu.dmi'
	icon_state = "zeng_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/torso/zenghu_panel
	name = "Zeng-Hu Spirit Torso Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_zenghu.dmi'
	icon_state = "zeng_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/arms/zenghu_panel
	name = "Zeng-Hu Spirit Arm Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_zenghu.dmi'
	icon_state = "zeng_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/legs/zenghu_panel
	name = "Zeng-Hu Spirit Leg Panel Colors"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_zenghu.dmi'
	icon_state = "zeng_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/nexus
	name = "Morpheus Nexus Colored Monitor"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "machine_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/torso/nexus
	name = "Morpheus Nexus Colored Torso"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "machine_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/arms/nexus
	name = "Morpheus Nexus Colored Arms"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "machine_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/legs/nexus
	name = "Morpheus Nexus Colored Legs"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "machine_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/raxus
	name = "Morpheus Raxus Colored Head"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "raxus_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/raxus_lights
	name = "Morpheus Raxus Head Lights"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "raxus_lights"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/hoplan
	name = "Morpheus Hoplan Colored Head"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "hoplan_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/indricus
	name = "Morpheus Indricus Colored Head"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "indricus_primary"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/marking/ipc/head/indricus
	name = "Morpheus Indricus Head Lights"
	icon = 'mods/_fd/fd_assets/icons/ipc/markings_baseline.dmi'
	icon_state = "indricus_lights"
	blend = ICON_MULTIPLY

// IPC SCREENS

/datum/sprite_accessory/facial_hair/ipc/scroll
	do_coloration = TRUE

/datum/sprite_accessory/facial_hair/ipc/dot
	do_coloration = TRUE

/datum/sprite_accessory/facial_hair/ipc/four
	do_coloration = TRUE

/datum/sprite_accessory/facial_hair/ipc/eye
	do_coloration = TRUE

/datum/sprite_accessory/facial_hair/ipc/eyes
	do_coloration = TRUE

/datum/sprite_accessory/facial_hair/ipc/pink
	name = "Pink"
	icon_state = "ipc_pink"

/datum/sprite_accessory/facial_hair/ipc/console
	name = "Console"
	icon_state = "ipc_console"

/datum/sprite_accessory/facial_hair/ipc/cancel
	name = "Cancel"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_cancel"

/datum/sprite_accessory/facial_hair/ipc/confirm
	name = "Confirm"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_confirm"

/datum/sprite_accessory/facial_hair/ipc/blue_eye
	name = "Blue mono eye"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_eye"

/datum/sprite_accessory/facial_hair/ipc/heartrate
	name = "Heartrate"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_heartrate"

/datum/sprite_accessory/facial_hair/ipc/lumi_eyes
	name = "Lumi eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_lumi_eyes"

/datum/sprite_accessory/facial_hair/ipc/music
	name = "Music"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_music"

/datum/sprite_accessory/facial_hair/ipc/rainbow
	name = "Rainbow"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_rainbow"

/datum/sprite_accessory/facial_hair/ipc/rainbowdiag
	name = "Diagonal rainbow"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_rainbowdiag"

/datum/sprite_accessory/facial_hair/ipc/nt
	name = "NT"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_nt"

/datum/sprite_accessory/facial_hair/ipc/zenghu
	name = "Zeng-hu"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_zenghu"

/datum/sprite_accessory/facial_hair/ipc/eridani
	name = "Eridani"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_eridani"

/datum/sprite_accessory/facial_hair/ipc/slotmachine
	name = "Slot machine"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_slotmachine"

/datum/sprite_accessory/facial_hair/ipc/question_many
	name = "Many questions"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_question"

/datum/sprite_accessory/facial_hair/ipc/staticrainbow
	name = "Rainbow static"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_static3"

/datum/sprite_accessory/facial_hair/ipc/caution
	name = "Caution"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_caution"

/datum/sprite_accessory/facial_hair/ipc/cautionblack
	name = "Caution black"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_caution2"

/datum/sprite_accessory/facial_hair/ipc/face
	name = "Face"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_face"

/datum/sprite_accessory/facial_hair/ipc/facecig
	name = "Face with cigarette"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_facecig"

/datum/sprite_accessory/facial_hair/ipc/star
	name = "Star"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_star"
	do_coloration = TRUE

/datum/sprite_accessory/facial_hair/ipc/starinverse
	name = "Inversed star"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_starinverse"
	do_coloration = TRUE

/datum/sprite_accessory/facial_hair/ipc/coin
	name = "Coin"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_coin"

/datum/sprite_accessory/facial_hair/ipc/crosshair
	name = "Crosshair"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_crosshair"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/temple
	name = "Temple"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_temple"

/datum/sprite_accessory/facial_hair/ipc/ouroboros
	name = "Ouroboros"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_ouroboros"

/datum/sprite_accessory/facial_hair/ipc/orbit
	name = "Orbit"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_orbit"

/datum/sprite_accessory/facial_hair/ipc/orbit2
	name = "Orbit 2"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_orbit2"

/datum/sprite_accessory/facial_hair/ipc/eyesspin
	name = "Spinning eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_eyes"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/eyesnospin
	name = "Eyes (No Spin)"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_eyes_noanim"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/buffering
	name = "Buffering"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_buffering"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/eyesgreen
	name = "Green eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_green_eyes"

/datum/sprite_accessory/facial_hair/ipc/eyesred
	name = "Red eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_red_eyes"

/datum/sprite_accessory/facial_hair/ipc/eyesyellow
	name = "Yellow eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_yellow_eyes"

/datum/sprite_accessory/facial_hair/ipc/eyespink
	name = "Pink eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_pink_eyes"

/datum/sprite_accessory/facial_hair/ipc/eyesorange
	name = "Orange eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_orange_eyes"

/datum/sprite_accessory/facial_hair/ipc/eyesblue
	name = "Blue eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_blue_eyes"

/datum/sprite_accessory/facial_hair/ipc/eyesfortuna
	name = "Fortuna eyes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_eyes_fortuna"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/nyaru
	name = "Nyaru"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_nyaru"

/datum/sprite_accessory/facial_hair/ipc/badapple
	name = "Bad apple"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_bad_apple"

/datum/sprite_accessory/facial_hair/ipc/textdrop
	name = "Text drop"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_textdrop"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/loading
	name = "Loading"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_loading"

/datum/sprite_accessory/facial_hair/ipc/windowsxp
	name = "XP"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_windowsxp"

/datum/sprite_accessory/facial_hair/ipc/windows
	name = "Windows"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_win"

/datum/sprite_accessory/facial_hair/ipc/coffee
	name = "Coffee"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_coffee"

/datum/sprite_accessory/facial_hair/ipc/aquarium
	name = "Aquarium"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_aquarium"

/datum/sprite_accessory/facial_hair/ipc/microwave
	name = "Microwave"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_microwave"

/datum/sprite_accessory/facial_hair/ipc/equalizer
	name = "Equalizer"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_equalizer"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/tamagotchi
	name = "Tamagotchi"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_tamagotchi"

/datum/sprite_accessory/facial_hair/ipc/smiley_green
	name = "Green smiley"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_smiley"

/datum/sprite_accessory/facial_hair/ipc/humanface
	name = "Human face"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_litso"

/datum/sprite_accessory/facial_hair/ipc/redtext
	name = "Red text"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_redtext"

/datum/sprite_accessory/facial_hair/ipc/ring
	name = "Ring"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_ring"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

/datum/sprite_accessory/facial_hair/ipc/l
	name = "L"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_screens.dmi'
	icon_state = "ipc_l"
	do_coloration = TRUE
	blend = ICON_MULTIPLY

// IPC HAIRS

/datum/sprite_accessory/hair/ipc/New()
	. = ..()
	name = "IPC " + name

/datum/sprite_accessory/hair/ipc/ipc_cross
	name = "Cross"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "cross"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_langle
	name = "Left Angle"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "langle"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_rangle
	name = "Right Angle"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "rangle"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_sidepanels
	name = "Side Panels"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "sidepanels"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_curved
	name = "Curved Antennas"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "curvedantennae"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_straight
	name = "Straight Antennas"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "straightantennae"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_circle
	name = "Circle Antennas"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "circleantenna"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_tusk
	name = "Tusk Antennas"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "tusk"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_horncrown
	name = "Horn Crown Antennas"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "horncrown"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_horns
	name = "Horn Antennas"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "dual_horn"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_rhorn
	name = "Right Horn Antenna"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "rhorn"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_lhorn
	name = "Left Horn Antenna"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "lhorn"
	blend = ICON_MULTIPLY

/datum/sprite_accessory/hair/ipc/ipc_dish
	name = "Antenna Dishes"
	icon = 'mods/_fd/fd_assets/icons/ipc/ipc_antennas.dmi'
	icon_state = "dual_dish"
	blend = ICON_MULTIPLY

// DIONA MARKINGS

/datum/sprite_accessory/marking/diona_leaves
	name = "Diona Leaves"
	icon = 'mods/_fd/fd_assets/icons/aurora/markings_diona.dmi'
	icon_state = "diona_leaves"
	body_parts = list(BP_L_FOOT,BP_R_FOOT,BP_L_LEG,BP_R_LEG,BP_L_ARM,BP_R_ARM,BP_CHEST,BP_GROIN,BP_HEAD)
	species_allowed = list(SPECIES_DIONA)

/datum/sprite_accessory/marking/diona_leaves/thorns_head
	name = "Diona Thorns (Head)"
	icon_state = "diona_thorns"
	body_parts = list(BP_HEAD)

/datum/sprite_accessory/marking/diona_leaves/thorns_torso
	name = "Diona Thorns (Torso)"
	icon_state = "diona_thorns"
	body_parts = list(BP_HEAD, BP_CHEST, BP_GROIN)

/datum/sprite_accessory/marking/diona_leaves/flowers_head
	name = "Diona Flowers (Head)"
	icon_state = "diona_flowers"
	body_parts = list(BP_HEAD)

/datum/sprite_accessory/marking/diona_leaves/flowers_torso
	name = "Diona Flowers (Torso)"
	icon_state = "diona_flowers"
	body_parts = list(BP_HEAD, BP_CHEST, BP_GROIN)

/datum/sprite_accessory/marking/diona_leaves/moss
	name = "Diona Moss"
	icon_state = "diona_moss"
	body_parts = list(BP_HEAD, BP_CHEST, BP_GROIN)

/datum/sprite_accessory/marking/diona_leaves/mushroom
	name = "Diona Mushroom"
	icon_state = "diona_mushroom"
	body_parts = list(BP_HEAD)

/datum/sprite_accessory/marking/diona_leaves/mushroom/antennae
	name = "Diona Antennae"
	icon_state = "diona_antennae"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes
	name = "Bug Eyes"
	icon_state = "bugeyes"
	body_parts = list(BP_HEAD)
	do_coloration = FALSE

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/human_eyes
	name = "Human Eyes"
	icon_state = "humaneyes"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/skrell_eyes
	name = "Skrell Eyes"
	icon_state = "skrelleyes"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/skrell_eyes_2
	name = "Skrell Eyes 2"
	icon_state = "skrelleyes2"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/small_horns
	name = "Small Horns"
	icon_state = "smallhorns"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/horny
	name = "Horny"
	icon_state = "horny"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/treebeard
	name = "Treebeard"
	icon_state = "treebeard"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/tinyeye
	name = "Tiny Eye"
	icon_state = "tinyeye"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/eyebrow
	name = "Eyebrow"
	icon_state = "eyebrow"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/bullhorn
	name = "Bullhorn"
	icon_state = "bullhorn"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/mono_eye
	name = "Mono Eye"
	icon_state = "monoeye"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/trioptics
	name = "Trioptics"
	icon_state = "trioptics"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/lopsided
	name = "Lopsided"
	icon_state = "lopsided"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/helmethead
	name = "Helmethead"
	icon_state = "helmethead"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/blinkinghelmethead
	name = "Blinking Helmethead"
	icon_state = "blinkinghelmethead"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/eyestalk
	name = "Eyestalk"
	icon_state = "eyestalk"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/periscope
	name = "Periscope"
	icon_state = "periscope"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/glorp
	name = "Glorp"
	icon_state = "glorp"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/oak
	name = "Oak"
	icon_state = "oak"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/thorns
	name = "Thorns"
	icon_state = "thorns"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/stump
	name = "Stump"
	icon_state = "stump"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/snout
	name = "Snout"
	icon_state = "snout"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/headtails
	name = "Head tails"
	icon_state = "headtails"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/headtails2
	name = "Head tails 2"
	icon_state = "headtails2"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/spanish_moss
	name = "Spanish Moss"
	icon_state = "spanishmoss"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/fun_guy
	name = "Fun Guy"
	icon_state = "funguy"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/red_cap
	name = "Red Cap"
	icon_state = "redcap"

/datum/sprite_accessory/marking/diona_leaves/bug_eyes/mellow_cap
	name = "Mellow Cap"
	icon_state = "mellowcap"

/datum/sprite_accessory/marking/diona_leaves/pbody
	name = "P-Body"
	icon_state = "pbody"
	body_parts = list(BP_HEAD, BP_CHEST)
	do_coloration = FALSE

/datum/sprite_accessory/marking/diona_leaves/pbody/blinking
	name = "Blinking P-Body"
	icon_state = "blinkingpbody"

/datum/sprite_accessory/marking/diona_leaves/foot_eye
	name = "Foot Eye"
	icon_state = "footeye"
	body_parts = list(BP_L_FOOT)
	do_coloration = FALSE

/datum/sprite_accessory/marking/diona_leaves/hand_eyes
	name = "Hand Eyes"
	icon_state = "handeye"
	body_parts = list(BP_R_HAND, BP_L_HAND)
	do_coloration = FALSE

/datum/sprite_accessory/marking/diona_leaves/tail
	name = "Diona Tail"
	icon_state = "dionatail"
	body_parts = list(BP_GROIN)
	do_coloration = FALSE
