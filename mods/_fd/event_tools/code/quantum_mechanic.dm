...
*/
// Код и Ассеты взяты из Небулы, с небольшими правками.
...
*/
	var/singleton/hierarchy/outfit/outfit = GET_SINGLETON(/singleton/hierarchy/outfit/quantum)
	outfit.equip(Q, OUTFIT_ADJUSTMENT_ALL_SKIPS)
...
*/
	Q.set_invisibility(INVISIBILITY_OBSERVER)
	to_chat(Q, SPAN_NOTICE("Invisimin on. You are now as invisible as a ghost. Press F8 to disengage that"))
	Q.alpha = max(mob.alpha - 100, 0)
...
/singleton/hierarchy/outfit/quantum
	...
	uniform =  /obj/item/clothing/under/quantum
	shoes =    /obj/item/clothing/shoes/black/quantum
	l_ear =    /obj/item/device/radio/headset/ert/quantum
...
/mob/living/carbon/human/quantum/New(loc, ...)
	. = ..()
...
// Gloves
/obj/item/clothing/gloves/insulated/black/quantum
...
/obj/item/storage/belt/utility/full/quantum/Initialize()
	...
	new /obj/item/clothing/gloves/insulated/black/quantum(src)
