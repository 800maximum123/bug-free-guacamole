...
/mob/living/simple_animal/fd/lancer/lancaster/resupply()
	. = ..()
	restock_charges = initial(restock_charges)

/mob/living/simple_animal/fd/lancer/lancaster
...
/mob/living/simple_animal/fd/lancer/lancaster/proc/choose_resupp(params)
	...
	switch(chosen_option)
		...
		if("Patch Allie/Self")
			if(restock_charges <= 0)
				...
				return FALSE
			for(var/mob/living/simple_animal/fd/lancer/M in view(2,src))
				if(!M.dead)
					mechs_in_radius += M
			var/mob/living/simple_animal/fd/lancer/target_choice = show_radial_menu(src, src, mechs_in_radius, radius = 30, require_near = TRUE, offset_x = 105, offset_y = 90)
			if(!target_choice)
				return FALSE
		...
		if("Restock Allie")
			if(restock_charges <= 0)
				to_chat(src, SPAN_WARNING("Для этого действия нужен как минимум 1 Заряд Пополнения!"))
				return FALSE
			for(var/mob/living/simple_animal/fd/lancer/M in oview(1,src))
...
/mob/living/simple_animal/fd/lancer/lancaster/ClickOn(atom/A, params)
			...
			switch(chosen_option)
				...
				if("Ping Area")
					...
					if(!do_after(src, 2 SECONDS, do_flags = DO_BOTH_CAN_MOVE))
						return FALSE
					for(var/mob/living/simple_animal/fd/lancer/M in oview(6,src))
