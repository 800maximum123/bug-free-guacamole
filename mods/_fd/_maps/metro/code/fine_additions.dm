...
// Новые записки От Канариса вкожено Тимохой

/datum/interactive_note/house_in_forest
	name = "Домик в лесу"
	note_info = {"Мы уже как десять лет не видимся с тобой. Но я вспоминаю каждые минуты, проведенные с тобой. Я не знаю зачем я это пишу и...\
				Зачем я сюда каждый год прихожу. Мы уже никогда не встретимся, однако я надеюсь, что ты смотришь на меня с облаков, гордясь мною.\
				Но... Это все же наше общее место, где мы мечтали и смотрели в небо и я... Сейчас смотрю туда, надеясь когда-то попасть к тебе. Я тебя люблю, Долорес. Твой "Принц". "}

/datum/interactive_note/radiocomp_first
	name = "Черновик"
	note_info = {"Черновик, оставленный на мантикорнете. Связи нету, поэтому прошлые сообщения не открываются, как и профиль того, кто хотел отправить это сообщение:\
				иду к тебе. Я тебя найду и вытащу. Знай это. Твоё имя не забыто, как и мое. Я иду по следу, оставленному тобой. Как по мне, так и по тебе ещё рано звонить колоколу. "}

/datum/interactive_note/radiocomp_second
	name = "НАДПИСЬ"
	note_info = {"На весь экран выскакивает надпись:\
				Я не знаю кто вы, но если вы видели это сообщение, то не ищите меня. Я растворился в посмертном закате луны."}

/datum/interactive_note/shef
	name = "Записка с хорошим подчерком"
	note_info = {"Эти идиоты заперли меня и поменяли замки. Когда я отсюда выберусь, то всех, ВСЕХ поставлю к стенке. Они будут знать как водиться со мной.\
				Я уже как неделю тут сижу. Электричество пропало, я попытался руками открыть двери, однако они не поддаются. Благо, вода в бачке унитаза ещё есть и у меня есть шанс отсюда выбраться.\
				Эта шутка затянулась...\
				Я чувствую, что потихоньку схожу с ума здесь... Кто-нибудь помогите. Я не хочу умирать так...\
				Неразборчиво"}

/datum/interactive_note/nummy
	name = "? ? ?"
	note_info = {"Чешется. Вкусно."}

/datum/interactive_note/centroom
	name = "Вернувшимся с вылазки!"
	note_info = {"Численность населения бункера сильно упала.\
				Смысла оставаться здесь нету. Мы должны сменить локацию, чтобы не быть задавленными под натиском Терры.\
				Если вы вернулись с вылазки и не видите людей вокруг, то не пугайтесь. Мы пошли дальше по путям, которые куда-то ведут.\
				Ещё эта болезнь, которая косит с практически полной летальностью... Говорили же не играться с инопланетными игрушками, но нет, этому ебучему НИО же надо... "}

/datum/interactive_note/kanarys_dead
	name = "НЕ ТРОГАЙТЕ ОРУЖИЕ ВОСХОЖДЕНИЯ"
	note_info = {"У нас нету возможности лечить болезнь. Практически всё НИО оказалось в крематории. Мы думали, что снаряды восхождения не такая уж и проблема, однако она оказалась намного серьезнее, чем мы ожидали.\
				При операции этот... Я даже не знаю как назвать... Яд? Просто выплеснулся и облил практических всех моих помощников. Все они в скором времени слегли.\
				Люди собираются отсюда уходить и перебираться в более безопасные места. Я тоже думаю, что мне стоит поступить также.\
				Ещё слышала, что какой-то шутник поменял нашему начальнику код на двери и теперь он будет сидеть там, пока не подберет код, что даст нам времени уйти отсюда. Без его брюзжаний.\
				Я, честно, не совсем помню, что там у него поставлено, но вроде что-то вроде... 2 3 * 9 (Одна цифра была кем-то стерта)"}

/datum/interactive_note/manytermins
	name = "Отчёт #0084"
	note_info = {"Идет куча медицинских терминов, которые трудно разобрать из-за почерка и сложности наименований, однако последние строчки были более разборчивы.\
				В местных условиях купированию и лечению пациент не подлежит. Направлен в лаборатории НИИ-68 для изучения и возможной реабилитации пациента."}
