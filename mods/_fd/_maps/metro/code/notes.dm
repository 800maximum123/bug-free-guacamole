...
/datum/interactive_note/gasmask1
	name = "Фильтры"
	note_info = {"<span style="color: yellow;">НАПОЛНИТЕЛЬ </span> - испытательный вариант фильтров против ядовитых паров Htz-23. Один фильтр способен эксплуатироваться в течении <span style="color: green;">пары минут.</span> <br>
	Время может варьироваться от степени концетрации ядовитых паров. <br>
	P.s. Мы уже знаем, что этого времени мало, но в скором времени должны появится более совершенные системы защиты дыхательных путей, наберитесь терпения. "}
...
/datum/interactive_note/brigade
	name = "Новые приказы"
	note_info = {"Мэдсен приказал всем начать полную изоляцию от внешнего мира, теперь мы сами по себе. Последняя беседа с майором должно быть поставила его в патовую ситуацию. Ничего, благо у нас достаточно ресурсов что-бы обеспечить себя как минимум на год беззаботного проживания тут. <br> Кто-то шутит что Терра будет искать нас словно тех парней с Земли. Которые укопались в джунглях с огромной сетью туннелей. Ну-ну."}
