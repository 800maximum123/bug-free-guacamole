...
/proc/shakeship(sstrength, stime, drop, osound = TRUE, region)
	for(var/mob/living/carbon/current_mob in GLOB.human_mobs)
		if (!get_z(current_mob) in GetConnectedZlevels(region))
			continue
		...
		if(sstrength > 2 && sstrength <= 7)
			...
			if(current_mob.client && osound)
				...
				sleep(1 SECOND)
				sound_to(current_mob.client, 'sound/effects/doorcreaky.ogg')
		if(sstrength > 7)
			if(current_mob.client && osound)
			...
			if(current_mob.client && osound)
				sleep(1 SECOND)
				sound_to (current_mob.client, 'pry2.ogg')
				sleep(1 SECOND)
				sound_to (current_mob.client, 'double_klaxon.ogg')
