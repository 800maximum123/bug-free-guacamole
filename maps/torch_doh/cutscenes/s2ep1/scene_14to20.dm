...
/datum/modular_cutscene/s2ep1sc14/setup_actions(...)
	actions = list(
		...
		TALK_ACTOR(actor("Ветерок - Сцена 14"), "В чём. Дело?") = 4 SECONDS,
		PLAY_SOUND(sound('maps/torch_doh/cutscenes/sounds/shorter_siren.ogg', volume = 5)),
...
/datum/modular_cutscene/s2ep1sc15/setup_actions(...)
	actions = list(
		...
		PLAY_SOUND(sound('maps/torch_doh/cutscenes/sounds/shorter_siren.ogg', volume = 5)),
		TALK_ACTOR(actor("Ашира - Сцена 15"), "На врряд ли, даже если бы я и выжила-...мне довелось бы в-встрретить кого-то врроде тебя ещё раз.") = 10 SECONDS,
